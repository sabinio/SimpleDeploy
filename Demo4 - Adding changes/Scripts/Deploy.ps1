...
Write-verbose $("Change scripts - Start")

foreach ($file in gc "$PSScriptRoot\..\changes\Changes.txt"){
    $file = gi "$PSScriptRoot\..\changes\$file"
#foreach ($file in gci "$PSScriptRoot\..\changes\*.sql"){
    Write-verbose $("Applying script {0}" -f $file.name)
    Invoke-Sqlcmd -ServerInstance "$ServerInstance" -Database $database -InputFile $file.FullName    
}
Write-verbose $("Change scripts - Finsihed")
